...
internal sealed class ConditionCodeRegistry
{
    ...
    public void Initialize(List<EncodableDefinition> definitions)
    {
        ...
        {
            var isApxCc =
                (definition.Instruction.Opcode >> 4) == 4 &&
                definition.Instruction.OpcodeMap == OpcodeMap.MAP4 &&
                definition.MandatoryPrefix == MandatoryPrefix.None; // TODO: Mark them with DB annotations
            var sccFilter = definition.GetIntFilter(SelectorDefinitions.EvexScc, -1);
            if (!isApxCc && sccFilter == -1)
            {
                continue;
            }
