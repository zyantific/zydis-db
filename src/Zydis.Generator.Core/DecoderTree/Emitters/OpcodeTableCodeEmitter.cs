    ...
    OpcodeTableEmitter
{
    ...
    protected override void EmitDecisionNode(DecisionNode node, IEnumerable<(int Index, DecoderTreeNode? TargetNode, int TargetAddress, int OffsetToTarget)> targets)
    {
        ...
        //    ? $"/* {node.Definition.GetSlotName(0),Padding} */, {node.Arguments[0].Data}"
        //    : "0";
        ...
        // Emit offsets to child nodes.

        foreach (var (index, targetNode, _, offsetToTarget) in targets)
        {
            _writer.WriteInlineComment($"{node.Definition.GetSlotName(index),Padding}");
            ...
            _writer.WriteExpression("ZYDIS_DT_OFFSET(0x{0:X4}) /* {1} */", offsetToTarget, targetNode);
        }
    }
    ...
    protected override void EmitDefinitionNode(DefinitionNode node)
    {
        ...
        var encodingId = _encodingRegistry.GetEncodingId(node.InstructionDefinition);
        ...
        _writer.WriteExpression("ZYDIS_DT_DEFINITION(0x{0:X4}) /* {1} */",
            _definitionRegistry.GetDefinitionId(node.InstructionDefinition),
            node.InstructionDefinition.Mnemonic);
    }
